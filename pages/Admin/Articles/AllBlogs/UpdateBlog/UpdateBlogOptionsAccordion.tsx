...
import {
  ...
  UpdateBlogMutationReturnData
} from "../../../../../@types/interfaces/PageInterfaces/Admin/Articles/UpdateBlog/updateblogaccordion.interfaces";
...
import UpdateBlogInformation from "./UpdateBlogInformation";
import Error from "../../../../../components/Error/Error";

const UpdateBlogOptionsAccordion: React.FC<Props & RouteComponentProps> = ({
  content,
  setContent,
  history,
  location,
  ...
  // tags input states
  const [tags, setTags] = useState<string>(blog.tags.toString());
  // category states
  const [category, setCategory] = useState<any>(blog.category);
  // blog image states
  const [imgUrl, setImgUrl] = useState<string>(blog.img);
  ...
  const onUpdateBlog: IAddBlogFunc = (
    e: React.MouseEvent<HTMLButtonElement, MouseEvent>
  ): void => {
    e.preventDefault();

    const variables: UpdateBlogMutationVariables = {
      blog_id: blogId,
    ...
    updateBlog({ variables }).then(({ data }) => {
      if (data.updateBlog.errorMessage !== "No error.") {
        setError(data.updateBlog.errorMessage);
      } else {
        setError(null);
      }
      resetInputValues();
      history.push(`/blog/details/${blog.id}`);
    });
  };
  ...
  return (
    <>
      ...
      <Button
        color="teal"
        loading={loading}
        onClick={(e: React.MouseEvent<HTMLButtonElement>) => onUpdateBlog(e)}
        content="Güncelle"
      />
...
export default withRouter(UpdateBlogOptionsAccordion);
